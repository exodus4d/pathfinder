...
if ! mysql -u"$MYSQL_USER" -p"$MYSQL_PASS" -h"$MYSQL_HOST" -e "use pathfinder_clean" --silent; then
        echo "Creating pathfinder_clean database"
        mysql -u"$MYSQL_USER" -p"$MYSQL_PASS" -h"$MYSQL_HOST" -e "create database pathfinder_clean;"
        mysql -u"$MYSQL_USER" -p"$MYSQL_PASS" -h"$MYSQL_HOST" pathfinder_clean < /home/$USER/pathfinder/pathfinder.sql
fi
